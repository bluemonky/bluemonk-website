...
13. **差し色（オレンジ）＝二灯化(2026-06-16)**: 「まだのっぺり」への対応。寒色シアン一色の平坦さを、暖色オレンジ `--ember(#ff8a3c)` を差し色に入れて温度差で解消。(a) **画面固定の二灯アンビエント層 `.bm-ambient`**（寒色シアン＋暖色オレンジを右上↔左下の対角で配置）を本文背面に敷き、全スクロール位置で奥行きと差し色を届かせる。(b) **主要CTAをオレンジ塗り `.btn-ember`**（Header「相談する」・Contact「メールで相談する」）= 寒色基調の中の焦点。(c) カードhoverに暖色の下グロー＋スイープ、(d) PROOF実績ドットを暖色＋微グロー、(e) HERO下部にモンクを持ち上げる暖色光。基調はシアンのまま・差し色は焦点に限定（発光する禅の節度維持）。**全セクションを実画面検証・本番ビルドOK。** ※強度は `.bm-ambient` の rgba と `--ember` で調整可。
